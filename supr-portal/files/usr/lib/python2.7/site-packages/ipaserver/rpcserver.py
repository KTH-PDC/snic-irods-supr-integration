...
class test(Backend, HTTP_Status):
    ...
    def _on_finalize(self):
        super(test, self)._on_finalize()
        self.api.Backend.wsgi_dispatch.mount(self, self.key)

    def __call__(self, environ, start_response):
        self.info('WSGI test.__call__:')
        ...
        # start building the response
        self.info("WSGI test: start test with user '%s'", data['user'])
        status = HTTP_STATUS_SUCCESS
        response_headers = [('Content-Type', 'text/html; charset=utf-8')]
        title = 'Test rejected'
        result = 'error'
        ...
        try:
            ...
            message = "Could not connect to LDAP server."
            self.error("test: cannot authenticate '%s' to LDAP server: %s",
                    data['user'], str(e))
        else:
            try:
                result = 'ok'
        ...
        if result == 'ok':
            cmdout = subprocess.check_output([ "/usr/bin/ipa-test.sh" ], stderr=subprocess.STDOUT)

        response_headers.append(('X-IPA-Test-Result', result))
        start_response(status, response_headers)
	if result == 'ok':
            output = _success_template % dict(title=str(title),
                                          message=str(cmdout))
...
# !!!! Portal class, our addition.
# Call password change.

class portal(Backend, HTTP_Status):
    ...
    def _on_finalize(self):
        super(portal, self)._on_finalize()
        self.api.Backend.wsgi_dispatch.mount(self, self.key)
    ...
    def __call__(self, environ, start_response):
        ...
        # Got query string parse it to a dictionary.
        data = {}
        # Debug info.
        #self.info("portal: Query string is '%s'", query_string)
        # Assemble the dictionary of arguments.
        # Session cookie, password to set and verification.
        for field in ('session', 'password', 'verification'):
            value = query_dict.get(field, None)
            if value is not None:
                data[field] = value[0]
            else:
        ...
        # start building the response
        self.info("WSGI test: start portal")
        status = HTTP_STATUS_SUCCESS
        response_headers = [('Content-Type', 'text/html; charset=utf-8')]
        ...
        se = data['session']
        pw = data['password']
        ve = data['verification']

        # Portal script.
        portal_script = '/usr/bin/supr-portal.py'

        self.info('%s: %s %s', "portal", "Change password, cookie is ", se)
        # Debug info.
        #self.info("%s %s %s %s ", portal_script, se, pw, ve)
        if pw != ve:
            return self.bad_request(environ, start_response,
                "Verification password did not match")
        if (len(pw) > 256 or len(ve) > 256):
            return self.bad_request(environ, start_response,
                "Password unreasonably long")

        # Calling out in a subprocess.
        message = "Portal exit call failed, unknown error"
        result = 'failure'
        try:

            # !!!! Call the script passing arguments.
            cmdout = subprocess.check_output(
                [ portal_script, se, pw, ve ],
                stderr=subprocess.STDOUT)
            title = "Portal request result"
            message = "Portal exit call was successful"
            result = 'ok'
        except subprocess.CalledProcessError as e:

            # There was an error so report it.
            message = "Portal exit call failed with code " + str(e.returncode)
            result = 'failure' + str(e.returncode)

        response_headers.append(('X-IPA-Portal-Result', result))
        start_response(status, response_headers)
        if result == 'ok':
            output = _success_template % dict(title=str(title),
                                          message=str(cmdout))
            ...
            # Return failure, which is the default message.
            output = _success_template % dict(title=str(title),
                                          message=str(message))
        return [output]
